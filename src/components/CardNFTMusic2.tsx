...
const CardNFTMusic2: FC<CardNFTMusic2Props> = ({
  ...
  const renderDefaultBtnListen = (state?: "loading" | "playing") => {
    return <span className="w-10 h-10 sm:w-11 sm:h-11 flex items-center justify-center rounded-full bg-white dark:bg-neutral-900/50 text-primary-6000 dark:text-primary-200 shadow-lg cursor-pointer">{renderIcon(state)}</span>;
  };

  return (
    <div className={`nc-CardNFTMusic2 relative flex justify-between p-2 space-x-2 rounded-3xl bg-neutral-100 dark:bg-neutral-800 hover:shadow-xl transition-shadow ${className}`} data-nc-id="CardNFTMusic2">
      <Link to={"/project-details"} className="flex-grow flex space-x-4">
        <div className="relative w-16 sm:w-24">
          <NcImage containerClassName="absolute inset-0 rounded-2xl overflow-hidden shadow-lg " src={featuredImage} />
        </div>
        ...
        <div className="flex flex-col justify-center flex-grow">
          ...
          <div className=" flex items-center pt-3 mt-1.5">
            {renderAvatars()}
            <Prices price="1.00 ETH" labelText="Price" className="sm:ml-3.5" contentClass="py-1.5 px-2 sm:px-3 text-xs sm:text-sm font-semibold" labelTextClassName="bg-neutral-100 dark:bg-neutral-800 " />
            <span className="block ml-3.5 text-neutral-500 dark:text-neutral-400 text-xs">1 of 100</span>
          </div>
        </div>
      </Link>

      <ButtonPlayMusicRunningContainer nftId={DEMO_NFT_ID} className="flex items-center" renderDefaultBtn={() => renderDefaultBtnListen()} renderLoadingBtn={() => renderDefaultBtnListen("loading")} renderPlayingBtn={() => renderDefaultBtnListen("playing")}></ButtonPlayMusicRunningContainer>
