...
const CardNFTVideo: FC<CardNFTVideoProps> = ({ className = "", isLiked, featuredImage = nftsImgs[1] }) => {
  const [DEMO_NFT_ID] = React.useState(nanoid());

  const renderAvatars = () => {
    return (
      <div className="hidden sm:flex -space-x-1 ">
        <Avatar containerClassName="ring-2 ring-white dark:ring-neutral-900" sizeClass="h-5 w-5 text-sm" />
        <Avatar containerClassName="ring-2 ring-white dark:ring-neutral-900" sizeClass="h-5 w-5 text-sm" />
        <Avatar containerClassName="ring-2 ring-white dark:ring-neutral-900" sizeClass="h-5 w-5 text-sm" />
        <Avatar containerClassName="ring-2 ring-white dark:ring-neutral-900" sizeClass="h-5 w-5 text-sm" />
      </div>
  ...
  const renderListenButtonDefault = (state?: "playing" | "loading") => {
    return <div className={`w-14 h-14 flex items-center justify-center rounded-full  cursor-pointer ${state === "playing" ? "bg-neutral-900/40 text-primary-50" : "bg-neutral-50/80 text-primary-500"}`}>{renderIcon(state)}</div>;
  };

  return (
    <div className={`nc-CardNFTVideo relative flex flex-col group ${className}`} data-nc-id="CardNFTVideo">
      <div className="relative flex-shrink-0 ">
        {/* AUDIO MEDiA */}
        <VideoForNft nftId={DEMO_NFT_ID} />

        <div className="">
          <NcImage containerClassName="flex aspect-w-16 aspect-h-9 w-full h-0 rounded-3xl overflow-hidden z-0" src={featuredImage} className="object-cover w-full h-full group-hover:scale-[1.03] transition-transform duration-300 ease-in-out will-change-transform" />
        </div>

        <LikeButton liked={isLiked} className="absolute top-3 right-3 z-10 !h-9" />

        <ButtonPlayMusicRunningContainer className="absolute bottom-3 left-3 z-10" nftId={DEMO_NFT_ID} renderDefaultBtn={() => renderListenButtonDefault()} renderPlayingBtn={() => renderListenButtonDefault("playing")} renderLoadingBtn={() => renderListenButtonDefault("loading")} />
      </div>

      <div className="p-5">
        <div className="flex justify-between items-center">
          <h2 className={`sm:text-lg font-semibold`}>NFT Video #{Math.floor(Math.random() * 1000) + 1000}</h2>
