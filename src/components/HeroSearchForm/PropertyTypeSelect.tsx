...
const typeOfProperty = [
  {
    name: "Pre-sale",
    description: "Projects needing funding to start",
    checked: true,
  },
  {
    name: "Released",
...
const PropertyTypeSelect: FC<PropertyTypeSelectProps> = ({ onChange, fieldClassName = "[ nc-hero-field-padding ]" }) => {
  return (
        ...
        <>
          ...
          >
            ...
            <div className="flex-grow">
              ...
              <span className="block mt-1 text-sm text-neutral-400 leading-none font-light ">Type of sale</span>
            </div>
          </Popover.Button>
                ...
                <div className="relative flex flex-col space-y-5">
                  {typeOfProperty.map((item) => (
                    <div key={item.name} className="">
                      <Checkbox name={item.name} label={item.name} subLabel={item.description} defaultChecked={item.checked} />
                    </div>
                  ))}
                </div>
              </div>
