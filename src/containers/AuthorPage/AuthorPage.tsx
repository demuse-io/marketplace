...
import { fetchPresale } from "contracts/presale.contract";
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      ...
      while (more) {
        ...
        const metadata = await fetchPresale(signer, i);
        console.log("result");
        if (metadata) {
                ...
                <Tab.Panel className="">
                  ...
                  >
                    {nfts &&
                      (nfts as []).length > 0 &&
                      nfts.map((nft) => {
                        {console.log("https://ipfs.io/ipfs/" + (nft as any).image)}
                        // {console.log((nft as any).image.replace('ipfs://', 'https://ipfs.io/ipfs/'))}
                       return <CardNFTMusic
                          featuredImage={"https://ipfs.io/ipfs/" + (nft as any).image}
                          className="sm:col-span-3 xl:col-span-2"
                          audioSrc={"https://ipfs.io/ipfs/" + (nft as any).image}
